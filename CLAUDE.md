...
1. **`docs/` 의 HTML 을 수정·삭제·이동하지 않는다.** 사용자가 직접 만든 자산이고, 파이프라인에게는 읽기 전용 입력이다. 내용이 잘못됐다고 판단해도 고치지 말고 보고한다.
...
5. **문서를 병합하거나 감추지 않는다.** 중복이 보이면 메타데이터(`related`, `overlaps`, `superseded_by`)로 관계만 표현한다.
...
| `wiki/entries/*.json` | AI + 사람 | `notes` 는 사람 전용 |
